...
class VersionFileTests(unittest.TestCase):
    ...
    def new(self, text):
        ...
        return stock.VersionFile(self.path)
    ...
    def test_qualify__dev2(self):
        ...
        assert str(vfile.v) == '0.1.2rc1.dev2'


class ToPyPiTests(unittest.TestCase):
    path = 'version.ini'
    command = stock.ToPyPI(test_mode=True)
    ...
    @mock.patch('bdd_coder.stock.ToPyPI.upload_cmd', return_value=['fake', 'line'])
    @mock.patch('bdd_coder.stock.ToPyPI.check_output')
    ...
    def test_aborted(self, stdout_mock, input_mock):
        ...
        input_mock.assert_called_once_with('Upload bdd-coder-1.1.0rc0 to PyPI (Yes/No)? ')
